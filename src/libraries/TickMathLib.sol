...
/// @title TickMathLib
/// @notice Computes sqrt price for ticks of size 1.0001, i.e. sqrt(1.0001^tick) as Q64.96
/// @dev Ported from Uniswap V3 / Aerodrome Slipstream TickMath for Solidity 0.8.24
library TickMathLib {
    ...
    /// @dev The minimum value that can be returned from getSqrtRatioAtTick (getSqrtRatioAtTick(MIN_TICK))
    uint160 internal constant MIN_SQRT_RATIO = 4295128739;
    /// @dev The maximum value that can be returned from getSqrtRatioAtTick (getSqrtRatioAtTick(MAX_TICK))
    ...
    /// @return sqrtPriceX96 A Q64.96 number representing the sqrt price
    ...
    /// @return tick The greatest tick for which the ratio is <= the input ratio
